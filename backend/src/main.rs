...
fn parse_nv_cmd(args: &[String]) -> Value {
    ...
    match sub.as_str() {
        "read" => {
            ...
            rpc::dispatch("nv read", &json!({ "path": rem[0], "slot": slot }))
        }
        "write" => {
            if rem.len() < 2 {
...
fn parse_bandlock_cmd(args: &[String]) -> Value {
    ...
    match sub.as_str() {
        ...
        "detect" => rpc::dispatch("bandlock detect", &json!({ "slot": slot })),
...
fn parse_features_cmd(args: &[String]) -> Value {
    ...
    match sub.as_str() {
        "check" => rpc::dispatch("features check", &json!({ "slot": slot })),
        "disable" => {
            if rem.is_empty() {
                return json!({ "ok": false, "error": "Usage: mtbctl features disable <id> [--slot N]" });
            }
            rpc::dispatch("features disable", &json!({ "id": rem[0], "slot": slot }))
        }
        "restore" => {
            if rem.is_empty() {
                return json!({ "ok": false, "error": "Usage: mtbctl features restore <id> [--slot N]" });
            }
            rpc::dispatch("features restore", &json!({ "id": rem[0], "slot": slot }))
        }
...
fn parse_cells_cmd(args: &[String]) -> Value {
    ...
    match sub.as_str() {
        "get" => rpc::dispatch("cells get", &json!({ "slot": slot })),
...
fn parse_modem_cmd(args: &[String]) -> Value {
    ...
    rpc::dispatch("modem restart", &json!({}))
}
...
fn parse_import_cmd(args: &[String]) -> Value {
    ...
    match sub.as_str() {
        "preview" => rpc::dispatch("import preview", &json!({ "json": json_input })),
        "apply" => rpc::dispatch("import apply", &json!({ "json": json_input })),
...
fn parse_backup_cmd(args: &[String]) -> Value {
    ...
    let sub = &args[0];
    match sub.as_str() {
        "list" => rpc::dispatch("backup list", &json!({})),
        "restore" => {
...
fn parse_config_cmd(args: &[String]) -> Value {
    ...
    let sub = &args[0];
    match sub.as_str() {
        "get" => rpc::dispatch("config get", &json!({})),
        "set" => {
            let json_input = match parse_json_input(&args[1..]) {
                Ok(s) => s,
                Err(e) => return json!({ "ok": false, "error": e }),
            };
            rpc::dispatch("config set", &json!({ "json": json_input }))
        }
...
fn parse_rpc_cmd(args: &[String]) -> Value {
    let mut payload: Option<String> = None;
    let mut i = 0;
    while i < args.len() {
        if args[i] == "--b64" && i + 1 < args.len() {
    ...
    match payload {
        Some(p) => rpc::rpc_exec(&p),
        None => json!({ "ok": false, "error": "Usage: mtbctl rpc --b64 <payload>" }),
    }
}
